...
- Each time the detector's reading drops (the user touched the machine), the
  window that just ended is folded into an exponential moving average of typical
  idle-window length. The count of windows seen is bounded.
...
- Confidence rises with the number of windows observed and falls once the
  current window runs past the typical window length (we are extrapolating).
...
DEFAULT_HISTORY = 32  # completed idle-window count the confidence saturates at
...
class IdlePredictor:
    ...
    def _record_window(self, length: float) -> None:
        """Fold a completed idle-window length into the bounded EWMA history."""
        self._window_count = min(self._window_count + 1, self._history)
    ...
    def _confidence(self, elapsed: float, mean: float) -> float:
        ...
        sample_conf = min(_FULL_CONFIDENCE, self._window_count / self._min_windows)
