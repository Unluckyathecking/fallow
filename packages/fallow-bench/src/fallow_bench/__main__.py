...
Exposes the workload, churn, experiment, and analysis commands. ``churn`` (B2)
loads a churn config (standalone or a B1
...
from fallow_bench.experiment.cli import RunFactory
from fallow_bench.experiment.cli import main as experiment_main
...
def main(
    ...
    *,
    experiment_runner_factory: RunFactory | None = None,
) -> None:
    args_list = sys.argv[1:] if argv is None else argv
    # B3 `analyze` owns its own argument parser; delegate before argparse below.
    if args_list and args_list[0] == _ANALYZE:
        raise SystemExit(analyze_main(list(args_list[1:])))
    if args_list and args_list[0] == _EXPERIMENT:
        raise SystemExit(experiment_main(args_list[1:], runner_factory=experiment_runner_factory))
    args = _parse_args(list(args_list))
